...
#
dlim $1 $region -t_srs $proj | gdal_query.py $2 -d_format "xyd" | gmt blockmedian $region -I$xinc/$yinc > _diff.xyz
#
#  make a difference grid
#
gmt blockmedian _diff.xyz $region -I$xinc/$yinc -rp -V | gmt surface $region -I$xinc/$yinc -G_diff.tif=gd+n-9999:GTiff -T1. -Z1.2 -V -rp -C.0001 -M50c
#
#  add the two grids
#
