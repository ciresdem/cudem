...
#waffles $region/-/-/.75/- $1 $2,200,1-e10 -E $xinc/$yinc -O dem_update -M surface:tension=1 -w -P $proj -T 1:2
waffles $region/-/-/1/- $1 -E $xinc/$yinc -O dem_update -M surface:tension=1:max_radius=${radius} -w -P $proj -T 1:2:split_value=0
...
##dlim $1 ${region}/-/-/1/- -t_srs $proj | gdal_query.py $2 -d_format "xyd" | \
 ##   gmt blockmedian $region -I$xinc/$yinc -V | \
   ## gmt surface $region -I$xinc/$yinc -G_diff.tif=gd+n-9999:GTiff -T1. -Z1.2 -V -rp -C.5 -Lu${max_diff} -Ll${min_diff} -M1c
